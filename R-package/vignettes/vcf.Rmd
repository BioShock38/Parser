...
dataset available here: [http://datadryad.org/resource/doi:10.5061/dryad.0817m](http://datadryad.org/resource/doi:10.5061/dryad.0817m). For computational reasons, we will only consider the first 1,000 variants by setting the argument `n` to `1000` in 
the `read.vcfR` function.

```
require(vcfR)
setwd("~/Downloads/Original_data_set_ch6_12_15/")
obj.vcfR <- read.vcfR("comt.chr06.snp.full.final.vcf",n=100)
...
vcf2pcadapt <- function(geno,output.file="file.pcadapt",blocksize=1000,console.count=10000){
  ...
  while (init.block < nSNP && end.block <= nSNP){
    ...
    hap2geno <- t(convert.line.2(snp.lines))
